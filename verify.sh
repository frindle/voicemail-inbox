...
# ALWAYS (re)install requirements so a dependency the task ADDS to
# requirements.txt (e.g. reportlab) reaches the venv, not just on first create.
if [ -x "$PY" ] && [ -f requirements.txt ]; then
