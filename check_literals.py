...
LITERALS = [['Dockerfile', 'apt-get install -y --no-install-recommends ffmpeg supervisor'], ['Dockerfile', 'COPY server.py worker.py supervisord.conf .'], ['Dockerfile', 'VOLUME ["/data", "/models"]'], ['Dockerfile', 'CMD ["supervisord", "-c", "/app/supervisord.conf"]'], ['supervisord.conf', 'nodaemon=true'], ['supervisord.conf', '[program:app]'], ['supervisord.conf', 'uvicorn server:app --host 0.0.0.0 --port 8000'], ['supervisord.conf', '[program:whisper]'], ['supervisord.conf', 'python worker.py'], ['supervisord.conf', 'autorestart=true'], ['docker-compose.yml', '10.0.12.44'], ['docker-compose.yml', '02:42:0A:00:0C:2C'], ['docker-compose.yml', '/mnt/user/data/Documents/Voicemail/whisper-models:/models'], ['requirements.txt', 'faster-whisper'], ['requirements.txt', 'requests']]
