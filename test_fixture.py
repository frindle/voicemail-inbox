...
CASES = [
    ...
    ("content-type is EXACTLY application/pdf (no trailing junk)",
     lambda: r_auth().headers.get("content-type", "").split(";")[0].strip() == "application/pdf", True),
    ("content-disposition is the exact attachment filename",
     lambda: r_auth().headers.get("content-disposition", "") == "attachment; filename=call-log.pdf", True),
